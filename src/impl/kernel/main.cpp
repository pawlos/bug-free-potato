...
struct pci_driver driver_table[] = {
    {0x04, 0x01, (driver_init_fn)ac97::init}
};
const int driver_count = 1;
constexpr pt::size_t MAX_DEVICES = 16;
void* devices[MAX_DEVICES];
pt::size_t devices_count = 0;

void initialize_pci_device(const pci_device* devicePtr) {
    while (devicePtr != nullptr && devicePtr->vendor_id != 0xffff)
    {
        for (int i = 0; i < driver_count; i++) {
            pci_driver driver = driver_table[i];
            if (driver.class_code == devicePtr->class_code &&
                driver.subclass_code == devicePtr->subclass_code) {
                    klog("\t\tDriver: \n");
                    void *obj = driver.init(devicePtr);
                    if (devices_count < MAX_DEVICES)
                        devices[devices_count++] = obj;
                }
        }
        devicePtr += 1;
    }
