...
static inline FAT12State* fat12_state(File* f) {
    return reinterpret_cast<FAT12State*>(f->fs_data);
}
...
bool FAT12::open_file(const char* filename, File* file) {
    if (!mounted) {
        return false;
    }
...
pt::uint32_t FAT12::read_file(File* file, void* buffer, pt::uint32_t bytes_to_read) {
...
void FAT12::close_file(File* file) {
    if (file) {
        file->open = false;
    }
}
